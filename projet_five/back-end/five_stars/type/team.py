...
class TeamMutation:
    @strawberry.mutation
    def createTeam(self, Owner_id:int ,Team_name: str) -> str:
        try:
            result = conn.execute(Team.insert().values(owner_id=Owner_id,team_name=Team_name, nbr_User=0, max_Users=7))
            conn.commit()
            return "Team successfully created"
        except SQLAlchemyError as e:
            return "Error in createTeam: " + str(e)


    @strawberry.mutation
    def updateTeam(self, Id: int,Team_name: str, Nbr_User: int) -> str:
        try:
    ...
    @strawberry.mutation
    def removeTeam(self, Id: int) -> str:
        try:
